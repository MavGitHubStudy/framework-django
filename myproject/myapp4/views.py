...
py_handler = logging.FileHandler(f"log/{__name__}.log", mode='a+', encoding='utf-8')
...
def user_form(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            py_logger.info(f'Получили {name=}, {email=}, {age=}.')
    else:
        form = UserForm()
...
def many_fields_form(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            # Делаем что-то с данными
            py_logger.info(f'Получили {form.cleaned_data=}.')
    else:
        # form = ManyFieldsForm()
